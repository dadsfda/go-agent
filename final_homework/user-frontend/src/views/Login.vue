...
async function handleAuth(mode) {
  ...
  try {
    const data = await request(`/auth/${mode}`, {
      method: 'POST',
      body: JSON.stringify({ role: 'candidate', email: email.value.trim(), password: password.value })
    })
...
<template>
  <div class="login-card">
    <div class="login-brand">
      <div class="login-icon">求</div>
      <h1>候选人登录</h1>
      <p>登录后可投递岗位、编辑档案、上传简历</p>
    </div>
    <form @submit.prevent="handleAuth('login')">
      <div class="form-group">
        <label>邮箱</label>
        <input v-model="email" type="email" placeholder="候选人邮箱" />
      </div>
      <div class="form-group">
        <label>密码</label>
